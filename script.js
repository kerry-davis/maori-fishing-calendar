...
function calculateBiteTimes(date, lat, lon) {
    ...
    const solunarTimes = solunar.getTimes(date, lat, lon);
    const formatBite = (bite) => ({
        start: bite.start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false }),
        end: bite.end.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false }),
        quality: 'good' // Default quality, can be improved later
    });
    return {
        major: solunarTimes.major ? solunarTimes.major.map(formatBite) : [],
        minor: solunarTimes.minor ? solunarTimes.minor.map(formatBite) : []
    };
}
